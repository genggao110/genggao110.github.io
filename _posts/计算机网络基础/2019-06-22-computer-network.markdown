...
2. 标识字段：占用16比特。用来唯一地标识主句发送的每一份数据报，通常每发送一份报文，它的值会加1
...
当客户端和服务器通过三次握手建立了TCP连接以后，当数据传送完毕，肯定是要断开TCP连接的啊。那对于TCP的断开连接，这里就有了神秘的“四次分手”。**由于TCP连接是全双工的，因此每个方向都必须单独进行关闭。这原则是当一方完成它的数据发送任务后就能发送一个FIN来终止这个方向的连接。**收到一个FIN只意味着这一方向上没有数据流动，一个TCP连接咋收到一个FIN后仍能发送数据。首先进行关闭的一方将主动执行关闭，而另一方执行被动关闭。
...
**第四次挥手**： A收到B的连接释放报文段后，对此发出确认报文段(ACK = 1, seq = u+1, ack = w + 1)，A进入TIME_WAIT(时间等待)状态。此时TCP未释放掉，需要进过时间等待计时器设置的时间2MSL后，A才进入CLOSED状态。
...
- SYN Cache技术。这种技术在收到 SYN 时不急着去分配系统资源,而是先回应一个ACK报文,并在一个专用的 HASH 表中(Cache)中保存这种半开连接,直到收到正确的ACK报文再去分配系统。
