...
![方法引用](https://tva2.sinaimg.com/large/005CDUpdgy1g522ltv9ahj30lz06jmzj.jpg)
...
![java内存模型](https://tva2.sinaimg.com/large/005CDUpdgy1g54ydcutebj30mo0dctdb.jpg)
...
![volatile写操作](https://tva2.sinaimg.com/large/005CDUpdgy1g557insoz7j30y20ns40t.jpg)
...
![volatile读](https://tva2.sinaimg.com/large/005CDUpdgy1g557lppd08j30u00jk76z.jpg)
...
[java 轻量级同步volatile关键字简介与可见性有序性与synchronized区别 多线程中篇（十二）](https://www.cnblogs.com/noteless/p/10410368.html)
...
![Java内部类](https://tva2.sinaimg.com/large/005CDUpdly1g58cc4m270j31sx175h37.jpg)
...
![Exception in Java](https://tva2.sinaimg.com/large/005CDUpdgy1g58fow23waj30hz0ghdgi.jpg)
...
```
![字节码信息](https://tva2.sinaimg.com/large/005CDUpdly1g58r7c8vw3j30z00nkdkh.jpg)
...
![类的加载时机](https://tva2.sinaimg.com/large/005CDUpdly1g59ve0nlq2j30ie05wmyd.jpg)
...
![类加载过程](https://tva2.sinaimg.com/large/005CDUpdly1g5a61x1thrj30ne06vmxx.jpg)
...
![类加载器之间的层次关系](https://tva2.sinaimg.com/large/005CDUpdgy1g5b4szbhnaj30gm0d2abd.jpg)
...
Spring使用**线程上下文类加载器**来解决这个问题。Spring加载类所用的classLoader都是通过Thread.currentThread().getContextClassLoader()获取的。当线程创建时会默认创建一个AppClassLoader类加载器（对应Tomcat中的WebAppclassLoader类加载器）： setContextClassLoader(AppClassLoader)。 利用这个来加载用户程序。即任何一个线程都可通过getContextClassLoader()获取到WebAppclassLoader。

**参考博文**：

[JVM(四)—一道面试题搞懂JVM类加载机制](https://blog.csdn.net/noaman_wgs/article/details/74489549)

[【Java面试题】之类加载：从面试题分析Java类加载机制](https://blog.csdn.net/zjkC050818/article/details/78376195)

[jvm之java类加载机制和类加载器(ClassLoader)的详解](https://blog.csdn.net/m0_38075425/article/details/81627349)

[Java虚拟机类加载机制和双亲委派模型](https://blog.csdn.net/xu768840497/article/details/79175335)
...
![JVM执行程序流程](https://tva2.sinaimg.com/large/005CDUpdgy1g5b9kpyxxjj30hs0asaag.jpg)
...
![JVM内存区域](https://tva3.sinaimg.com/large/005CDUpdgy1g5b9p999b9j30hs0b4t99.jpg)
...
![内存结构](https://tva2.sinaimg.com/large/005CDUpdly1g5ba7jpo95j30hs09vwev.jpg)
...
![父子类方法表中存储过程](https://tva2.sinaimg.com/large/005CDUpdgy1g5baaxqzm8j30hs0aeglu.jpg)
...
![Object类](https://tva2.sinaimg.com/large/005CDUpdgy1g5bty0rjrtj30wx0pjtam.jpg)
...
![ObjectMonitor结构](https://tva2.sinaimg.com/large/005CDUpdgy1g5c5o1n31uj30iq0bfwef.jpg)
...
![ObjectMonitor](https://tva2.sinaimg.com/large/005CDUpdgy1g5c66vaq0zj30et08v0t8.jpg)
...
**ObjectWaiter**
![ObjectWaiter](https://tva2.sinaimg.com/large/005CDUpdgy1g5c6ax57f7j30j50ajaa3.jpg)

ObjectWaiter对象是双向链表结构，保存了_thread（当前线程）以及当前的状态TState等数据， 每个等待锁的线程都会被封装成ObjectWaiter对象。

**wait方法实现**

lock.wait()方法最终通过ObjectMonitor的`void wait(jlong millis, bool interruptable, TRAPS);`实现：
1. 将当前线程封装成ObjectWaiter对象node;

![step1](https://tva2.sinaimg.com/large/005CDUpdgy1g5c6xss5a8j30qa0jz75v.jpg)

2. 通过`ObjectMonitor::AddWaiter`方法将node添加到_WaitSet列表中：

![step2](https://tva2.sinaimg.com/large/005CDUpdgy1g5c71abr20j30g10aqglm.jpg)

3. 通过`ObjectMonitor::exit`方法释放当前的ObjectMonitor对象，这样其它竞争线程就可以获取该ObjectMonitor对象。

![step3](https://tva2.sinaimg.com/large/005CDUpdgy1g5c72x1shsj30i50dt755.jpg)
...
lock.notify()方法最终通过ObjectMonitor的`void notify(TRAPS)`实现：
1. 如果当前_WaitSet为空，即没有正在等待的线程，则直接返回；
2. 通过`ObjectMonitor::DequeueWaiter`方法，获取_WaitSet列表中的第一个ObjectWaiter节点，实现也很简单。(**在jdk的notify方法注释是随机唤醒一个线程，其实是第一个ObjectWaiter节点**)

![notify()](https://tva2.sinaimg.com/large/005CDUpdgy1g5c75qt0l6j30is0iw74i.jpg)

3. 根据不同的策略，将取出来的ObjectWaiter节点，加入到_EntryList或则通过`Atomic::cmpxchg_ptr`指令进行自旋操作cxq。


**notifyAll方法实现**

lock.notifyAll()方法最终通过ObjectMonitor的`void notifyAll(TRAPS)`实现：通过for循环取出_WaitSet的ObjectWaiter节点，并根据不同策略，加入到_EntryList或则进行自旋操作。
...
[Java 中的 Monitor 机制](https://www.jianshu.com/p/7f8a873d479c)

[Java的wait()、notify()学习三部曲之一：JVM源码分析](https://blog.csdn.net/boling_cavalry/article/details/77793224)

[java的Object里wait()实现原理](https://blog.csdn.net/ZytheMoon/article/details/89461016)

[Java中Object.wait()、Object.notify()/notifyAll()底层原理](https://blog.csdn.net/u013096592/article/details/80019871)

### 4.11 深拷贝与浅拷贝

首先明白的是，浅拷贝与深拷贝都是针对一个已有对象的操作。我们先来了解下浅拷贝与深拷贝的概念。

在Java中，除了**基本数据类型(元类型)**之外，还存在类的实例对象这个引用数据类型。而一般使用'='号做赋值操作的时候，对于基本类型而言，实际上拷贝的是它的值，但是对于对象而言，其实赋值的只是这个对象的引用，将原对象的引用传递过去，它们实际上还是指向的同一个对象。
...
所以，所谓的浅拷贝与深拷贝，只是在拷贝对象的时候，对于**类的实例对象**这种引用数据类型的不同操作而已。
...
![浅拷贝](https://tva2.sinaimg.com/large/005CDUpdgy1g7qj197k8gj30ka0bp3yi.jpg)
...
![深拷贝](https://tva2.sinaimg.com/large/005CDUpdgy1g7qk1hkv5jj30ka0bp0sr.jpg)
...
[细说Java的深拷贝和浅拷贝](https://segmentfault.com/a/1190000010648514)
